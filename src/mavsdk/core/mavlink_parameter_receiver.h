...
/**
 * One could also call this MavlinkParameterProvider or MavlinkParameterServer but all these names
 * don't really fit perfectly. This class makes it easy to accomplish the following task: Having a
 * component that exposes some settings for a user to configure. In General, the usage of this class
 * is as following: 1) provide parameters (now other components can request and change these
 * parameters via mavlink). 2) add listener(s) for these parameters such that you can react to
 * changes set by another component. 3) let mavlink do its magic. Note that this side is much more
 * simple - it does not need to worry about re-transmission or such, the responsibility for that
 * lies completely on the part that wants to change parameters. Regarding non-extended and extended
 * parameters protocol: In addition to the parameter types from the non-extended parameter protocol,
 * the extended parameter protocol also supports string parameter values. This class supports
 * clients using both the non-extended and the extended parameter protocol, but hides the string
 * parameter values from non-extended protocol clients. Therefore, if the server has std:.string
 * parameters but is talking to a non-extended client, param_index and param_count are different
...
public:
     ...
     * It is recommended to not change the parameter set after the first communication with any
     * client. (https://mavlink.io/en/services/parameter_ext.html#parameters_invariant).
     ...
     * Result::WrongType if the same parameter name is provided with a different type (aka updating
     * the parameter would mutate the type of an already provided parameter),
     * Result::ParamValueTooLong if the parameter type is std::string but the value is longer than
     ...
     * @tparam T the type of the parameter to retrieve, if the parameter from the parameter set does
     * not match this type, the method will return MAVLinkParameters::Result::WrongType  and the
     * value is default constructed.
     * @param name the name of the parameter to retrieve, if the parameter set does not contain this
     * name key MAVLinkParameters::Result::NotFound is returned and the value is default constructed
     * @return MAVLinkParameters::Result::Success if the name is a valid key for the parameter set,
     * AND the type matches the value in the set. Otherwise,one of the error codes above.
     */
...
private:
     ...
     * @param extended true if the message is coming from the extended protocol,false otherwise. The
     * response workflow is slightly different on the extended protocol.
     */
     ...
     * @param is_request we also respond to MAV_COMP_ID_ALL on messages that are a "request", but
     * not on the "set" messages.
     */
